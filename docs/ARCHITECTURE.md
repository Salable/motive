...
| `MotiveUI` | AppKit/SwiftUI surfaces. | `SpriteHost` (engine ↔ SwiftUI bridge), `SpriteView` (atlas renderer), `SpriteBoxWindow` (chat, action buttons, bubbles), `NotificationMenu`, `SettingsWindow` (capability-driven, `extraSections` for custom panes). |
