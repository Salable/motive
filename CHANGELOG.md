...
### Added
- **A starter sprite kit.** `Sprites/starter/` ships two MIT-licensed packages
  on one grid (8×9 cells of 128×128): `pip`, a complete animated companion
  covering the whole lifecycle vocabulary, and `template`, the same grid drawn
  as guides — safe area, centre line, ground line, and each cell's state and
  frame number — to draw over. Both are generated by
  `scripts/make-starter-sprites.py` and pinned by `StarterKitTests`. Run one
  with `MOTIVE_SPRITE=Sprites/starter/pip swift run motive-demo`.
